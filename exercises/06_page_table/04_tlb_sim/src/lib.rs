...
impl Tlb {
    ...
    pub fn insert(&mut self, vpn: u64, ppn: u64, asid: u16, flags: u64) {
        ...
        if let Some(entry) = self.entries.iter_mut().find(|entry| {
            ...
            self.entries[self.fifo_ptr] = TlbEntry { valid: true, asid, vpn, ppn, flags };
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        let tlb_lookup = self.tlb.lookup(vpn, self.current_asid);
        if tlb_lookup.is_some() {
            return Some(tlb_lookup.unwrap());
        }
        for (asid, page_mapping) in &self.page_table {
            if *asid == self.current_asid && page_mapping.vpn == vpn {
                self.tlb.insert(vpn, page_mapping.ppn, *asid, page_mapping.flags);
                return Some(page_mapping.ppn);
            }
        }
        None
    }
