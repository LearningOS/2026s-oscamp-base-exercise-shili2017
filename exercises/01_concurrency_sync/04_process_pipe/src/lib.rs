...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    let stdout = Command::new(program).args(args).stdout(Stdio::piped()).output().unwrap().stdout;
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    let exit_status = Command::new("sh").args(["-c", command]).status().unwrap();
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    let mut child = Command::new("grep").arg(pattern).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().unwrap();
    let mut stdin = child.stdin.take().unwrap();
    stdin.write_all(input.as_bytes()).unwrap();
    drop(stdin);
    let mut result = String::new();
